#This code was developed together with Martin Lempp.
...
features = np.array(pd.read_excel (r'graph_structure.xlsx','Features_discrete'))

#set hyperparameters and conditions
cross_val_sets      = 5
negative_rate       = 1
lr                  = 0.0001
weight_decay        = 1e-3
...
#yes: only metabolite information, no: metabolite + flux information
if only_mets == 'yes':  
    features[314:,2:] = 0
...
#perform negative sampling (go to function for more details)
...
for k in range(N_samples):
    #do train and validation multiple times and save the models
    ...
    for cs in range(cross_val_sets-1): # take all sets except the last one
        ...
        y           = edge_list[2,:] #regulation vs backbone
        y2          = edge_list[3,:] #positive vs negative samples
        ...
        for epoch in range(1, epochs+1):
           ...
           TrainAccLoss['Loss'].append(loss.cpu().item())
           loss.backward()
           optimizer.step()
        ...
        find_uniques = unique_analyse(uniques, pred1.cpu().detach(), data_val.cpu().detach())
...
saving  = '_epochs_'+str(epochs)+'_'+CONV_or_LIN
...
#setup backbone of network + specific regulations of the test set
#basically the backbone is the same for all models
orig_ids                = ((edges_and_negatives[2,:] + edges_and_negatives[3,:]) == 1)
orig_backbone           = edges_and_negatives[:,orig_ids]
...
x           = np.matrix(features)
x           = torch.tensor(x, dtype = torch.float) 
test_edges_orig = torch.tensor(test_edges_orig, dtype = torch.int64)
edge_index  = edge_list[0:2,:]
y           = edge_list[2,:]
data_test   = Data(x = x, edge_index = edge_index,edge_index_orig = test_edges_orig[0:2,:], y = y)
#submit model to cuda and some hyperparameters
...
for k in range(N_samples):
    ...
    for i, model in enumerate(m):
        ...
        _, pred1 = out1.max(dim = 1)
        test_result.append(np.array(pred1.cpu().detach()))
